#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2020/2/17 14:13
# @Author  : lwjnicole
# @File    : listEmployeeByUuids_test.py
import requests
import logging
import unittest
...
class listValidEmployeeByUuids(unittest.TestCase):
    ...
    def test_list_ValidEmployee_by_uuids(self):
        """根据uuids查询员工信息（不包含删除的)"""
        property_list = PullResourceUuidList()
        self.requests_url = self.baseUrl + '/employee/listValidEmployeeByUuids'
        self.payload = {
            "uuids": [
        ...
        self.result = r.json()
        self.assertTrue(len(self.result['data']), msg='接口没有查到员工')
